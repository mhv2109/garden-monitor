...
#define READ_ERROR_CHECK(err) switch(err) { \
	case ESP_OK: \
		break; \
...
#define WRITE_ERROR_CHECK(err) if (err != ESP_OK) { \
	ESP_LOGE(TAG, "Error (%s) writing to NVS", esp_err_to_name(err)); \
	return false; \
}

static const char *TAG = "nvs_component"; // logging tag
static bool NVS_INITIALIZED = false; // has the NVS flash system been initialized?

bool
read_string(const char *ns, const char *key, char *result)
{
	nvs_handle_t handle;
	size_t len;
...
bool
read_int8(const char *ns, const char *key, int8_t *result)
{
	nvs_handle_t handle;
